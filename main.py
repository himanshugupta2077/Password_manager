...
def view_saved_entry():
	...
	lbl1 = Label(second_frame, text = "Saved Entry", font=("Manjari Thin", 30), bg = "#23262B", fg="white")
	...
	for x in document_list:
		...
		button1 = Button(second_frame, text = title.title(), command = partial(title_viewer ,password), width = 20, bg = "#4658E0", fg="white", highlightthickness=0, borderwidth=0)
		button1.grid(row = a, column = 1, pady = 10, padx = 10)
		a += 1

		button1 = Button(second_frame, text = "Delete", command = partial(delete_me ,password), width = 10, bg = "#4658E0", fg="white", highlightthickness=0, borderwidth=0)
		button1.grid(row = b, column = 2, pady = 10, padx = 10)
		b += 1

		button1 = Button(second_frame, text = "Edit", command = partial(update_me ,password), width = 10, bg = "#4658E0", fg="white", highlightthickness=0, borderwidth=0)
...
def options():
	...
	button1 = Button(window, text = "Change master password", command = change_master_password, bg = "#4658E0", fg="white", highlightthickness=0, borderwidth=0)
...
def help():
	...
	lbl= Label(root1, text = "SafePass", font=("Great Vibes", 13), bg = "#060606", fg="white")
	lbl.place(x = 215, y = 170)
	lbl= Label(root1, text = "1.0", font=("Great Vibes", 13), bg = "#060606", fg="white")
	lbl.place(x = 241, y = 200)
	lbl= Label(root1, text = "SafePass keeps all your Passoword in a Digital Vault", font=("Great Vibes", 13), bg = "#060606", fg="white")
...
def first_window():
	...
	txt3 = Entry(window, width = 33, show = "●", bg="#1D1B19", fg="white", highlightthickness=0, borderwidth=0)
	...
	def check_password1():
		...
		if key != new_key: 
			tkinter.messagebox.showinfo("Unable to open vault!", "Sorry, that didn't work.\nPlease try again.")
		else: 
			password_vault()
		...
		def send_me_otp():
			...
			if entered_email == email:
				...
				def send_otp(otp):
					...
					message = f"""\
Subject: One Time Password (OTP) Confirmation - SafePass
...
"""
				...
				def check_otp():
					...
					else: tkinter.messagebox.showinfo("Warning!", "Incorrect OTP\nTry again")
				...
				button = Button(window, text = "Send again", command = just_send_it ,width = 8 , bg = "#4658E0", fg="white", highlightthickness=0, borderwidth=0)
			...
			else: tkinter.messagebox.showinfo("Warning!", "Enter Registered Email-ID\nTry again")
...
def new_identity():
	...
	def check_new_credentials():
		...
		if "".__eq__(txt1.get()) or "".__eq__(txt2.get()) or "".__eq__(txt3.get()) or "".__eq__(txt4.get()) or "".__eq__(txt5.get()):
			...
			tkinter.messagebox.showinfo("Error!", "Password should be longer than 8 characters. Please try again.")
			...
			tkinter.messagebox.showinfo("New ID created!", "Please login to continue.")
